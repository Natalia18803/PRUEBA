...
export const generarLecturaDiaria = async (req, res) => {
  try {
    ...
    // Verificar si el usuario está activo (Ahora dinámico)
    const activo = await usuario.isMembershipActive();
    if (!activo) {
      return res.status(403).json({ error: 'Usuario no tiene membresía activa o ha expirado' });
    }
